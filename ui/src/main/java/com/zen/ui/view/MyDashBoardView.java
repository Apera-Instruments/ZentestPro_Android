...
public class MyDashBoardView extends View {
     ...
     * 开始弧度
     */
    private int mStartAngle = 190;//135;
    /**
     * 圆弧扫过的弧度
     */
    ...
    private void drawGraduation(Canvas canvas){
        ...
        for(float i=0;i<=mSweepAngle;i+=avgLargeAngle){
    ...
    private void drawGraduation2(Canvas canvas){
        ...
        for(float i=0;i<=mSweepAngle;i+=avgSmallAngle){
            canvas.drawLine(point2[0],point2[1],point1[0],point1[1],mArcPaint);
    ...
    /*
    *
    * Bitmap bits = BitmapFactory.decodeResource(this.getResources(), R.drawable.ttt);
Bitmap bit2 = BitmapFactory.decodeResource(this.getResources(), R.drawable.sss);

canvas.drawBitmap(bit2,this.getWidth()/2-bit2.getWidth()/2, this.getHeight()/2-bit2.getHeight()/2, null);



    Matrix matrix = new Matrix();
    matrix.postTranslate(-bits.getWidth()/2, -bits.getHeight()/2);//步骤1
    matrix.postRotate(45);//步骤2
    matrix.postTranslate(this.getWidth()/2,this.getHeight()/2);//步骤3  屏幕的中心点
    canvas.drawBitmap(bits, matrix, null);//步骤4
    matrix.reset();
    *
    *
    * */
